...
class OtcbtcNeoethSpider(Spider):
    name = 'otcbtcNeoeth'
    allowed_domains = ['otcbtc.com']
    start_urls = [
	'https://otcbtc.com/sell_offers?currency=neo&fiat_currency=cny&payment_type=all&sort_by=most_trust',
	'https://otcbtc.com/buy_offers?currency=eth&fiat_currency=cny&payment_type=all&sort_by=most_trust',
        'https://bb.otcbtc.com/exchange/markets/neoeth'
	]
    custom_settings = {
        'ITEM_PIPELINES': {
            'CoinPairArbitrage.pipelines.pipelinesOtcbtcNeoeth.CoinpairarbitrageOtcbtcNeoethPipeline': 400
        }
    }
    profit_compute_dict = {}
    ...
    def parse_bb_item(self, response):
        ...
        bb_price_match=re.search(r'gon.ticker={"name":"NEO/ETH".+?last":"(.+?)",', first_script_item)
    ...
    def parse(self, response):
        ...
        if otc_currency_match:
            ...
            if len(self.profit_compute_dict) == 3:
                adjust_profit = (Decimal(10000) / self.profit_compute_dict['neo']) * self.profit_compute_dict['eth'] * self.profit_compute_dict['neoeth']
